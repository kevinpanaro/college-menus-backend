...
#############################################################
##                                                         ##
##        TO RUN: scrapy crawl menu_crawler_tomorrow       ##
##                                                         ##
#############################################################
#############################################################
...
class college_menus_spider(Spider):
    ...
    name = "menu_crawler_tomorrow"                                                                  # name of spider
    allowed_domains = ["www.drexelcampusdining.com"]                                                # only searches on this domain
    start_urls = ["http://www.drexelcampusdining.com/handschumacher.html"]                               # spider starts here
    ...
    def parse(self, response, *args, **kwargs):
        ...
        total_delta = datetime.date.today() + day_delta
        selected_date = [total_delta.strftime("%m/%d/%Y"),total_delta.strftime("%Y%m%d"),total_delta.strftime("%A").lower()]
        sel = Selector(response)

        if selected_date[2] is "monday":
            weeks = sel.xpath("//td[@bgcolor='#FFFFFF']/p[@align='center']/a/@href").extract()[1]       # Hans10.6.14.htm
            weeks_url = "http://www.drexelcampusdining.com/" + weeks     
        else:                               
            weeks_url = "http://www.drexelcampusdining.com/WeeklyMenu.htm"                              # FUCK YOU DREXEL
        
        yield Request(weeks_url, callback=self.parse_menus)
